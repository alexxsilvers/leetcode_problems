...
func main() {
	// 2
	log.Println(numUniqueEmails([]string{"test.email+alex@leetcode.com", "test.e.mail+bob.cathy@leetcode.com", "testemail+david@lee.tcode.com"}))
	// 1
	log.Println(numUniqueEmails([]string{"test.email+alex@leetcode.com", "test.email@leetcode.com"}))
}
...
func numUniqueEmails(emails []string) int {
	...
	for _, email := range emails {
		processedEmail := processEmail(email, &builder)
		uniqueEmails[processedEmail] = struct{}{}
		builder.Reset()
	}
...
func processEmail(email string, builder *strings.Builder) string {
	for i, char := range []byte(email) {
		switch char {
		case '.':
		case '+':
			builder.WriteString(email[strings.Index(email, "@"):])
			return builder.String()
		case '@':
			builder.WriteString(email[i:])
			return builder.String()
		default:
